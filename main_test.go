...
var _ = Describe("Task Tracker Plus", Ordered, func() {
	...
	Describe("API", func() {
		Describe("User API", func() {
			...
			//==========================================================
			//BATAS UNTUK MENUNJUKKAN ADANYA PERUBAHAN
			//==========================================================
			Describe("GetUserTaskCategory", func() {
				...
				When("sending without cookie", func() {
					It("should return status code 303", func() { // Ekspektasi diubah dari 401 ke 303
						r, _ := http.NewRequest("GET", "/api/v1/user/tasks", nil)
						w := httptest.NewRecorder()
						r.Header.Set("Content-Type", "application/json")
						apiServer.ServeHTTP(w, r)
						
						Expect(w.Code).To(Equal(http.StatusSeeOther)) // Middleware mengembalikan 303
						Expect(w.Header().Get("Location")).To(Equal("/login")) // Pastikan redirect ke login
					})
				})
